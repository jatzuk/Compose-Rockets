...
fun Renderer(scene: Scene) {
  val bitmapRocket = imageResource(Res.drawable.rocket)
  val rocketImage = Image.makeFromBitmap(bitmapRocket.asSkiaBitmap())
  val (rocketWidth, rocketHeight) = remember { Rocket.WIDTH to Rocket.HEIGHT }

  val bitmap = imageResource(Res.drawable.target).asSkiaBitmap()
  val targetImage = Image.makeFromBitmap(bitmap)
